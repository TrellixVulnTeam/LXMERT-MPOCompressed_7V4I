...
def FixCentralTensorCalculateAuxiliaryTensor(ori_tensor_set,ori_matrix,mpo_input_shape,mpo_output_shape,ranks):
    ...
    ori_matrix = torch.from_numpy(ori_matrix).cuda() if type(ori_matrix) is np.ndarray else ori_matrix.cuda()
    ...
    if type(ori_tensor_set[0]) is np.ndarray:
        ...
        e = ori_tensor_set[4]
        f = ori_tensor_set[5]
    B = torch.tensordot(a,b, ([3],[0])).reshape(-1,ranks[2]) # (i1j1i2j2,r2)
    C = c.reshape(-1,ranks[3]) # (r3,i3j3,r4), D = (r4,i4j4,r5)
    E = torch.tensordot(e,f, ([3],[0])).reshape(ranks[4], -1) # (r4,i4j4i5j5,r6)
    ...
    new_c = torch.matmul(new_cd, D_inv).reshape(ranks[2], mpo_input_shape[2],mpo_output_shape[2], ranks[3])
